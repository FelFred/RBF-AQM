...
#define r_table                 		op_sv_ptr->r_table
#define buf                     		op_sv_ptr->buf
#define r1_file                 		op_sv_ptr->r1_file
...
#define FIN_PREAMBLE_DEC	scheduler_TCP_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((scheduler_TCP_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
extern "C" {
#endif
	void scheduler_TCP (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_scheduler_TCP_init (int * init_block_ptr);
	void _op_scheduler_TCP_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_scheduler_TCP_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_scheduler_TCP_alloc (VosT_Obtype, int);
	void _op_scheduler_TCP_svar (void *, const char *, void **);
...
void
scheduler_TCP (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (scheduler_TCP ());

		{


		FSM_ENTER ("scheduler_TCP")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "scheduler_TCP [init enter execs]")
				FSM_PROFILE_SECTION_IN ("scheduler_TCP [init enter execs]", state0_enter_exec)
				{
				...
				if (feng || sally) {	
					...
					//w_q = 1 - exp(-1/link_cap);
					w_q = 0.001;  		 // RED avg queue size filter parameter
					//w_q = 0.001;
				}
				...
				*/
				...
				r1_file = fopen("C:\\Felipe\\routing1.txt","r");
				// initialize routing table to random number = 20
				for (i = 0; i < 10; i = i + 1){
					r_table[i][1] = 20; 
					r_table[i][2] = 20;
				}
				...
				if (r1_file == NULL){
					...
					printf("file pointer ISNT NULL\n");
				}
				
				while (fscanf(r1_file, "%s", buf) == 1){
					printf("read was correct\n");
					printf("buf is %s\n", buf);
				}
				...
				for (i = 0; i < 10; i = i + 1){
					printf("elements of row no %d are equal to = %d and %d\n", i, r_table[i][1], r_table[i][2]); 
				}
				*/
				}
			...
			FSM_STATE_EXIT_FORCED (0, "init", "scheduler_TCP [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_0", "scheduler_TCP [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "scheduler_TCP [idle enter execs]")
				FSM_PROFILE_SECTION_IN ("scheduler_TCP [idle enter execs]", state1_enter_exec)
				{
				if (PKTIN){
			...
			FSM_EXIT (3,"scheduler_TCP")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "idle", "scheduler_TCP [idle exit execs]")


			/** state (idle) transition processing **/
			FSM_PROFILE_SECTION_IN ("scheduler_TCP [idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, ;, "FINISH", "", "idle", "fin", "tr_2", "scheduler_TCP [idle -> fin : FINISH / ]")
				FSM_CASE_TRANSIT (1, 1, state1_enter_exec, ;, "default", "", "idle", "idle", "tr_1", "scheduler_TCP [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (2, "fin", state2_enter_exec, "scheduler_TCP [fin enter execs]")
				FSM_PROFILE_SECTION_IN ("scheduler_TCP [fin enter execs]", state2_enter_exec)
				{
				/*
				fclose(fp);
				fclose(fp_loss);
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"scheduler_TCP")


			/** state (fin) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "fin", "scheduler_TCP [fin exit execs]")
...
void
_op_scheduler_TCP_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_scheduler_TCP_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = __LINE__;
#endif

	FIN_MT (_op_scheduler_TCP_terminate ())
...
/* local variable prs_ptr in _op_scheduler_TCP_svar function. */
#undef r_table
#undef buf
#undef r1_file
#undef fp
...
VosT_Obtype
_op_scheduler_TCP_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_scheduler_TCP_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (scheduler_TCP)",
		sizeof (scheduler_TCP_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_scheduler_TCP_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	scheduler_TCP_state * ptr;
	FIN_MT (_op_scheduler_TCP_alloc (obtype))

	ptr = (scheduler_TCP_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "scheduler_TCP [init enter execs]";
#endif
		}
...
void
_op_scheduler_TCP_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	scheduler_TCP_state		*prs_ptr;

	FIN_MT (_op_scheduler_TCP_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (scheduler_TCP_state *)gen_ptr;

	if (strcmp ("r_table" , var_name) == 0)
		{
		*var_p_ptr = (void *) (prs_ptr->r_table);
		FOUT
		}
	if (strcmp ("buf" , var_name) == 0)
		{
		*var_p_ptr = (void *) (prs_ptr->buf);
		FOUT
		}
	if (strcmp ("r1_file" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->r1_file);
		FOUT
		}
