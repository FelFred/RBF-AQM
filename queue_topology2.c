...
#define WKSTN (port==0)||(port==2)
#define SERVER ((port==1)||(port==3))
#define BG (port == 4)
...
#define bg_destroyed_file       		op_sv_ptr->bg_destroyed_file
#define bg_loss_file            		op_sv_ptr->bg_loss_file
...
#define rtt_bg                  		op_sv_ptr->rtt_bg
...
#define drop_debt_s0            		op_sv_ptr->drop_debt_s0
#define drop_debt_s1            		op_sv_ptr->drop_debt_s1
#define drop_debt_s2            		op_sv_ptr->drop_debt_s2
#define drop_debt_s3            		op_sv_ptr->drop_debt_s3
#define drop_debt_s4            		op_sv_ptr->drop_debt_s4
#define drop_debt_s5            		op_sv_ptr->drop_debt_s5
#define drop_debt_s6            		op_sv_ptr->drop_debt_s6
#define drop_debt_s7            		op_sv_ptr->drop_debt_s7
...
#define last_ok_s0              		op_sv_ptr->last_ok_s0
#define last_ok_s1              		op_sv_ptr->last_ok_s1
#define last_ok_s2              		op_sv_ptr->last_ok_s2
#define last_ok_s3              		op_sv_ptr->last_ok_s3
#define last_ok_s4              		op_sv_ptr->last_ok_s4
#define last_ok_s5              		op_sv_ptr->last_ok_s5
#define last_ok_s6              		op_sv_ptr->last_ok_s6
#define last_ok_s7              		op_sv_ptr->last_ok_s7
#define drop_reg1               		op_sv_ptr->drop_reg1
#define drop_reg0               		op_sv_ptr->drop_reg0
#define drop_reg_s0             		op_sv_ptr->drop_reg_s0
#define drop_reg_s1             		op_sv_ptr->drop_reg_s1
#define drop_reg_s2             		op_sv_ptr->drop_reg_s2
#define drop_reg_s3             		op_sv_ptr->drop_reg_s3
#define drop_reg_s4             		op_sv_ptr->drop_reg_s4
#define drop_reg_s5             		op_sv_ptr->drop_reg_s5
#define drop_reg_s6             		op_sv_ptr->drop_reg_s6
#define drop_reg_s7             		op_sv_ptr->drop_reg_s7
...
#define nxt_idx_s0              		op_sv_ptr->nxt_idx_s0
#define nxt_idx_s1              		op_sv_ptr->nxt_idx_s1
#define nxt_idx_s2              		op_sv_ptr->nxt_idx_s2
#define nxt_idx_s3              		op_sv_ptr->nxt_idx_s3
#define nxt_idx_s4              		op_sv_ptr->nxt_idx_s4
#define nxt_idx_s5              		op_sv_ptr->nxt_idx_s5
#define nxt_idx_s6              		op_sv_ptr->nxt_idx_s6
#define nxt_idx_s7              		op_sv_ptr->nxt_idx_s7
...
#define last_lost_s0            		op_sv_ptr->last_lost_s0
#define last_lost_s1            		op_sv_ptr->last_lost_s1
#define last_lost_s2            		op_sv_ptr->last_lost_s2
#define last_lost_s3            		op_sv_ptr->last_lost_s3
#define last_lost_s4            		op_sv_ptr->last_lost_s4
#define last_lost_s5            		op_sv_ptr->last_lost_s5
#define last_lost_s6            		op_sv_ptr->last_lost_s6
#define last_lost_s7            		op_sv_ptr->last_lost_s7
...
#define FIN_PREAMBLE_DEC	scheduler_TCP_bkg_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((scheduler_TCP_bkg_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
double get_ttf_loss(double r_loss){
	double ttf_loss;
...
void get_file_str(const char* extra_str){
...
extern "C" {
#endif
	void scheduler_TCP_bkg (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_scheduler_TCP_bkg_init (int * init_block_ptr);
	void _op_scheduler_TCP_bkg_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_scheduler_TCP_bkg_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_scheduler_TCP_bkg_alloc (VosT_Obtype, int);
	void _op_scheduler_TCP_bkg_svar (void *, const char *, void **);
...
void
scheduler_TCP_bkg (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (scheduler_TCP_bkg ());

		{


		FSM_ENTER ("scheduler_TCP_bkg")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "scheduler_TCP_bkg [init enter execs]")
				FSM_PROFILE_SECTION_IN ("scheduler_TCP_bkg [init enter execs]", state0_enter_exec)
				{
				// Get rtt from links delay
				l11_id = op_id_from_name(0, OPC_OBJTYPE_LKDUP, "wkstn_0 <-> loss_queue");
				l12_id = op_id_from_name(0, OPC_OBJTYPE_LKDUP, "loss_queue <-> server_0");
				l21_id = op_id_from_name(0, OPC_OBJTYPE_LKDUP, "wkstn_1 <-> loss_queue");
				l22_id = op_id_from_name(0, OPC_OBJTYPE_LKDUP, "loss_queue <-> server_1");
				...
				// Get queue id 
				my_id = op_id_from_name(0, OPC_OBJMTYPE_NODE, "loss_queue");
				...
				link_cap = 100000000/12000; // Asume C=1e9 y mss = 1500 *8 = 1200. Resultado esta en pkts/s
				...
				bg_destroyed_file = fopen("C:\\bg_destroyed.txt","w+");
				bg_loss_file = fopen("C:\\bg_loss.txt","w+");
			...
			FSM_STATE_EXIT_FORCED (0, "init", "scheduler_TCP_bkg [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_0", "scheduler_TCP_bkg [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "scheduler_TCP_bkg [idle enter execs]")
				FSM_PROFILE_SECTION_IN ("scheduler_TCP_bkg [idle enter execs]", state1_enter_exec)
				{
				if (PKTIN){
					...
					if (BG){
						...
						// Get dest number to identify destroyed pkts data
						if (op_pk_nfd_access(pkt, "destination", &dest_addr) == OPC_COMPCODE_FAILURE){
							printf("access failed while trying to read at bg!\n");			
						}
						if (dest_addr == 4){
							if (fields_tcp->seq_num == 253578871){
								printf("QUEUE RECEIVED PKT OF INTEREST at time: %f\n", op_sim_time());	
							}
						}
						...
						switch (dest_addr){
								case 4:
									//drop_reg = drop_reg_s0;
									for (i = 0;i < L; i = i + 1)
										{
											drop_reg[i] = drop_reg_s0[i];
										}
									last_ok = last_ok_s0;
									nxt_idx = nxt_idx_s0;
									drop_debt = drop_debt_s0;
									last_lost = last_lost_s0;
									break;
						
								case 6:
									//drop_reg = drop_reg_s1;
									for (i = 0;i < L; i = i + 1)
										{
											drop_reg[i] = drop_reg_s1[i];
										}
									last_ok = last_ok_s1;
									nxt_idx = nxt_idx_s1;
									drop_debt = drop_debt_s1;
									last_lost = last_lost_s1;
									break;
				
								case 8:
									//drop_reg = drop_reg_s2;
									for (i = 0;i < L; i = i + 1)
										{
											drop_reg[i] = drop_reg_s2[i];
										}
									last_ok = last_ok_s2;
									nxt_idx = nxt_idx_s2;
									drop_debt = drop_debt_s2;
									last_lost = last_lost_s2;
									break;
				
								case 10:
									//drop_reg = drop_reg_s3;
									for (i = 0;i < L; i = i + 1)
										{
											drop_reg[i] = drop_reg_s3[i];
										}
									last_ok = last_ok_s3;
									nxt_idx = nxt_idx_s3;
									drop_debt = drop_debt_s3;
									last_lost = last_lost_s3;
									break;
				
								case 12:
									//drop_reg = drop_reg_s4;
									for (i = 0;i < L; i = i + 1)
										{
											drop_reg[i] = drop_reg_s4[i];
										}
									last_ok = last_ok_s4;
									nxt_idx = nxt_idx_s4;
									drop_debt = drop_debt_s4;
									last_lost = last_lost_s4;
									break;
				
								case 14:
									//drop_reg = drop_reg_s5;
									for (i = 0;i < L; i = i + 1)
										{
											drop_reg[i] = drop_reg_s5[i];
										}
									last_ok = last_ok_s5;
									nxt_idx = nxt_idx_s5;
									drop_debt = drop_debt_s5;
									last_lost = last_lost_s5;
									break;
				
								case 16:
									//drop_reg = drop_reg_s6;
									for (i = 0;i < L; i = i + 1)
										{
											drop_reg[i] = drop_reg_s6[i];
										}
									last_ok = last_ok_s6;
									nxt_idx = nxt_idx_s6;
									drop_debt = drop_debt_s6;
									last_lost = last_lost_s6;
									break;
				
								case 18:
									//drop_reg = drop_reg_s7;
									for (i = 0;i < L; i = i + 1)
										{
											drop_reg[i] = drop_reg_s7[i];
										}
									last_ok = last_ok_s7;
									nxt_idx = nxt_idx_s7;
									drop_debt = drop_debt_s7;
									last_lost = last_lost_s7;
									break;
						...
						if (cur_seq>last_ok){
							...
							switch (dest_addr){
								...
								default:
									printf("dest_addr: %d (deberia ser par entre 4 y 18)\n", dest_addr);				
							}
						...
						if (op_dist_uniform(1.0) > intrinsic_loss){
							if (op_subq_stat (subq_index, OPC_QSTAT_PKSIZE) < subq_size){
								...
								// INICIO: Obtener rtt del tráfico de fondo.
								op_pk_nfd_access(pkt, "measure RTT", &rtt_bg);
								//printf("RTT bg = %f\n", ((double) rtt_bg)/fact_conv);
								last_rtt = ((double) rtt_bg)/fact_conv;
									
								// FIN: Obtener rtt del tráfico de fondo.
								...
								if (drop_debt > 0){
									if (droppable){
										...
										switch (dest_addr){
											case 4:
												drop_debt_s0 = drop_debt_s0 - 1;
												break;				
											case 6:
												drop_debt_s1 = drop_debt_s1 - 1;
												break;
											case 8:
												drop_debt_s2 = drop_debt_s2 - 1;
												break;
											case 10:
												drop_debt_s3 = drop_debt_s3 - 1;
												break;
											case 12:
												drop_debt_s4 = drop_debt_s4 - 1;
												break;
											case 14:
												drop_debt_s5 = drop_debt_s5 - 1;
												break;
											case 16:
												drop_debt_s6 = drop_debt_s6 - 1;
												break;
											case 18:
												drop_debt_s7 = drop_debt_s7 - 1;
												break;
											default:
												printf("dest_addr: %d (deberia ser par entre 4 y 18)\n", dest_addr);			
										}
										// record in register and update related variables
										switch (dest_addr){
											case 4:	
												drop_reg_s0[nxt_idx_s0] = cur_seq;
												last_lost_s0 = cur_seq;
												nxt_idx_s0 = (nxt_idx_s0 < L-1) ? (nxt_idx_s0 + 1) : 0;
												break;				
											case 6:	
												drop_reg_s1[nxt_idx_s1] = cur_seq;
												last_lost_s1 = cur_seq;
												nxt_idx_s1 = (nxt_idx_s1 < L-1) ? (nxt_idx_s1 + 1) : 0;
												break;
											case 8:	
												drop_reg_s2[nxt_idx_s2] = cur_seq;
												last_lost_s2 = cur_seq;
												nxt_idx_s2 = (nxt_idx_s2 < L-1) ? (nxt_idx_s2 + 1) : 0;
												break;
											case 10:	
												drop_reg_s3[nxt_idx_s3] = cur_seq;
												last_lost_s3 = cur_seq;
												nxt_idx_s3 = (nxt_idx_s3 < L-1) ? (nxt_idx_s3 + 1) : 0;
												break;
											case 12:	
												drop_reg_s4[nxt_idx_s4] = cur_seq;
												last_lost_s4 = cur_seq;
												nxt_idx_s4 = (nxt_idx_s4 < L-1) ? (nxt_idx_s4 + 1) : 0;
												break;
											case 14:	
												drop_reg_s5[nxt_idx_s5] = cur_seq;
												last_lost_s5 = cur_seq;
												nxt_idx_s5 = (nxt_idx_s5 < L-1) ? (nxt_idx_s5 + 1) : 0;
												break;
											case 16:	
												drop_reg_s6[nxt_idx_s6] = cur_seq;
												last_lost_s6 = cur_seq;
												nxt_idx_s6 = (nxt_idx_s6 < L-1) ? (nxt_idx_s6 + 1) : 0;
												break;
											case 18:	
												drop_reg_s7[nxt_idx_s7] = cur_seq;
												last_lost_s7 = cur_seq;
												nxt_idx_s7 = (nxt_idx_s7 < L-1) ? (nxt_idx_s7 + 1) : 0;
												break;
											default:
												printf("dest_addr: %d (deberia ser par entre 4 y 18)\n", dest_addr);			
										}
										// drop						
										fprintf(bg_destroyed_file, "%d %d %f 0 %d\n", fields_tcp->seq_num, dest_addr, op_sim_time(), drop_debt);	
										op_pk_destroy(pkt);
									} else {
									...
									if ((cur_qsize > (!recommended_red) * threshold * subq_size) && (op_dist_uniform(1.0) <= var_loss)){
										if (droppable){		
											...
											// drop pkt
											fprintf(bg_destroyed_file, "%d %d %f 3\n", fields_tcp->seq_num, dest_addr, op_sim_time());	
											op_pk_destroy(pkt);
											red_count = 0;
										} else {
											increase_debt = 1;
								...
								fprintf(bg_destroyed_file, "%d %d %f 2\n", fields_tcp->seq_num, dest_addr, op_sim_time());
								op_pk_destroy(pkt);	
							}
						} else {
							fprintf(bg_destroyed_file, "%d %d %f 1\n", fields_tcp->seq_num, dest_addr, op_sim_time());
							op_pk_destroy(pkt);
						}
						...
						if (dest_addr == 4){
							fprintf(debt_debug_file, "%d %d %d %d %f %f %f %f %f %f %d %d %d %d\n", cur_seq, droppable, increase_debt, drop_debt, cur_qsize, var_loss, red_loss, aggr_coef, last_rtt, op_sim_time(), subq_size, repetition, might_drop, last_lost);
						}
					}
					...
					if ((SERVER) && (!flag_ttf)){ // toward workstation (data)
						...
						if (outcome == 0 || outcome == 1 || outcome == 2 || outcome == 3){
							...
							//fprintf(qdata_file, "%d %f %f %f %f 1 %f\n", pkts_inq, cur_qsize, q_delay, op_sim_time(), max_p, idle_stamp);
						...
						if (port == 1){
							//fprintf(debt_debug_file, "%d %d %d %d %d %f %f %f %f %f %f %d %d %d %d %d\n", cur_seq, droppable, outcome, increase_debt, drop_debt, cur_qsize, var_loss, red_loss, aggr_coef, last_rtt, op_sim_time(), subq_size, outcome4_instance, repetition, might_drop, last_lost);
						}
					...
					if (dest_addr <= 3){
						op_pk_send(pkt, dest_addr);  // serviced queued packet (data)
					...
					fprintf(qdata_file, "%d %f %f %f 0 %f %d\n", pkts_inq, cur_qsize, op_sim_time(), max_p, idle_stamp, red_count);
			...
			FSM_EXIT (3,"scheduler_TCP_bkg")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "idle", "scheduler_TCP_bkg [idle exit execs]")


			/** state (idle) transition processing **/
			FSM_PROFILE_SECTION_IN ("scheduler_TCP_bkg [idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, ;, "FINISH", "", "idle", "fin", "tr_2", "scheduler_TCP_bkg [idle -> fin : FINISH / ]")
				FSM_CASE_TRANSIT (1, 1, state1_enter_exec, ;, "default", "", "idle", "idle", "tr_1", "scheduler_TCP_bkg [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (2, "fin", state2_enter_exec, "scheduler_TCP_bkg [fin enter execs]")
				FSM_PROFILE_SECTION_IN ("scheduler_TCP_bkg [fin enter execs]", state2_enter_exec)
				{
				/*
				fclose(fp);
				fclose(fp_loss);
				...
				*/
				...
				printf("queue filter constant w_q = %f\n", w_q);
				}
				FSM_PROFILE_SECTION_OUT (state2_enter_exec)

			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"scheduler_TCP_bkg")


			/** state (fin) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "fin", "scheduler_TCP_bkg [fin exit execs]")
...
void
_op_scheduler_TCP_bkg_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_scheduler_TCP_bkg_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = __LINE__;
#endif

	FIN_MT (_op_scheduler_TCP_bkg_terminate ())
...
/* local variable prs_ptr in _op_scheduler_TCP_bkg_svar function. */
#undef fp
#undef fp_loss
#undef fp64
#undef fp_aggr
...
VosT_Obtype
_op_scheduler_TCP_bkg_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_scheduler_TCP_bkg_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (scheduler_TCP_bkg)",
		sizeof (scheduler_TCP_bkg_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_scheduler_TCP_bkg_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	scheduler_TCP_bkg_state * ptr;
	FIN_MT (_op_scheduler_TCP_bkg_alloc (obtype))

	ptr = (scheduler_TCP_bkg_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "scheduler_TCP_bkg [init enter execs]";
#endif
		}
...
void
_op_scheduler_TCP_bkg_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	scheduler_TCP_bkg_state		*prs_ptr;

	FIN_MT (_op_scheduler_TCP_bkg_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (scheduler_TCP_bkg_state *)gen_ptr;
	...
	if (strcmp ("bg_destroyed_file" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->bg_destroyed_file);
		FOUT
		}
	if (strcmp ("bg_loss_file" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->bg_loss_file);
		FOUT
		}
	...
	if (strcmp ("rtt_bg" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->rtt_bg);
		FOUT
		}
	...
	if (strcmp ("drop_debt_s0" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->drop_debt_s0);
		FOUT
		}
	if (strcmp ("drop_debt_s1" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->drop_debt_s1);
		FOUT
		}
	if (strcmp ("drop_debt_s2" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->drop_debt_s2);
		FOUT
		}
	if (strcmp ("drop_debt_s3" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->drop_debt_s3);
		FOUT
		}
	if (strcmp ("drop_debt_s4" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->drop_debt_s4);
		FOUT
		}
	if (strcmp ("drop_debt_s5" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->drop_debt_s5);
		FOUT
		}
	if (strcmp ("drop_debt_s6" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->drop_debt_s6);
		FOUT
		}
	if (strcmp ("drop_debt_s7" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->drop_debt_s7);
		FOUT
		}
	...
	if (strcmp ("last_ok_s0" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_ok_s0);
		FOUT
		}
	if (strcmp ("last_ok_s1" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_ok_s1);
		FOUT
		}
	if (strcmp ("last_ok_s2" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_ok_s2);
		FOUT
		}
	if (strcmp ("last_ok_s3" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_ok_s3);
		FOUT
		}
	if (strcmp ("last_ok_s4" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_ok_s4);
		FOUT
		}
	if (strcmp ("last_ok_s5" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_ok_s5);
		FOUT
		}
	if (strcmp ("last_ok_s6" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_ok_s6);
		FOUT
		}
	if (strcmp ("last_ok_s7" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_ok_s7);
		FOUT
		}
	...
	if (strcmp ("drop_reg_s0" , var_name) == 0)
		{
		*var_p_ptr = (void *) (prs_ptr->drop_reg_s0);
		FOUT
		}
	if (strcmp ("drop_reg_s1" , var_name) == 0)
		{
		*var_p_ptr = (void *) (prs_ptr->drop_reg_s1);
		FOUT
		}
	if (strcmp ("drop_reg_s2" , var_name) == 0)
		{
		*var_p_ptr = (void *) (prs_ptr->drop_reg_s2);
		FOUT
		}
	if (strcmp ("drop_reg_s3" , var_name) == 0)
		{
		*var_p_ptr = (void *) (prs_ptr->drop_reg_s3);
		FOUT
		}
	if (strcmp ("drop_reg_s4" , var_name) == 0)
		{
		*var_p_ptr = (void *) (prs_ptr->drop_reg_s4);
		FOUT
		}
	if (strcmp ("drop_reg_s5" , var_name) == 0)
		{
		*var_p_ptr = (void *) (prs_ptr->drop_reg_s5);
		FOUT
		}
	if (strcmp ("drop_reg_s6" , var_name) == 0)
		{
		*var_p_ptr = (void *) (prs_ptr->drop_reg_s6);
		FOUT
		}
	if (strcmp ("drop_reg_s7" , var_name) == 0)
		{
		*var_p_ptr = (void *) (prs_ptr->drop_reg_s7);
		FOUT
		}
	...
	if (strcmp ("nxt_idx_s0" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->nxt_idx_s0);
		FOUT
		}
	if (strcmp ("nxt_idx_s1" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->nxt_idx_s1);
		FOUT
		}
	if (strcmp ("nxt_idx_s2" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->nxt_idx_s2);
		FOUT
		}
	if (strcmp ("nxt_idx_s3" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->nxt_idx_s3);
		FOUT
		}
	if (strcmp ("nxt_idx_s4" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->nxt_idx_s4);
		FOUT
		}
	if (strcmp ("nxt_idx_s5" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->nxt_idx_s5);
		FOUT
		}
	if (strcmp ("nxt_idx_s6" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->nxt_idx_s6);
		FOUT
		}
	if (strcmp ("nxt_idx_s7" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->nxt_idx_s7);
		FOUT
		}
	...
	if (strcmp ("last_lost_s0" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_lost_s0);
		FOUT
		}
	if (strcmp ("last_lost_s1" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_lost_s1);
		FOUT
		}
	if (strcmp ("last_lost_s2" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_lost_s2);
		FOUT
		}
	if (strcmp ("last_lost_s3" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_lost_s3);
		FOUT
		}
	if (strcmp ("last_lost_s4" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_lost_s4);
		FOUT
		}
	if (strcmp ("last_lost_s5" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_lost_s5);
		FOUT
		}
	if (strcmp ("last_lost_s6" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_lost_s6);
		FOUT
		}
	if (strcmp ("last_lost_s7" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->last_lost_s7);
		FOUT
		}
